...
exports.studLogin = asyncHandler(async function (req, res) {
    const { email, password } = req.body
    if (!email || !password) {
        res.send({ status: 400, mssg: "Please Add email and Password" })
    }
    const stud = await userTechModel.userTech.findOne({ email })
    console.log(stud)
    if (!stud) {
        res.send({ status: 400, mssg: "User not found ,please signup" })
    }
    const passwordIsCorrect = await bcrypt.compare(password, stud.password)
    if (stud && passwordIsCorrect) {
        const token = generatedToken(stud._id)
        ...
        res.send({ status: 200, message: "Login successfully", data: stud })
    } else {
